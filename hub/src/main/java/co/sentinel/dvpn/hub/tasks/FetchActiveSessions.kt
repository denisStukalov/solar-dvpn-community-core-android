...
import sentinel.session.v1.Querier
import sentinel.session.v1.QueryServiceGrpc
import sentinel.types.v1.StatusOuterClass
import timber.log.Timber
import java.util.concurrent.TimeUnit

object FetchActiveSessions {
    suspend fun execute(account: Account) = kotlin.runCatching {
        ...
        val response = stub.querySessionsForAddress(
            Querier.QuerySessionsForAddressRequest.newBuilder()
